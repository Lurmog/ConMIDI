...
void setupntdll()
{
    if ((ntdll_libHandle = LoadLibrary("ntdll")) == NULL)
    {
        printf("Could not load ntdll\n\n");
        return;
    }
    ntdllAvailable = 1;
    if ((NtDelayExecution = (NtDE)GetProcAddress(ntdll_libHandle, "NtDelayExecution")) == NULL)
    {
        printf("GetProcAddress failed for ntdll NtDelayExecution.\n\n");
    }
    if ((NtQuerySystemTime = (NtQST)GetProcAddress(ntdll_libHandle, "NtQuerySystemTime")) == NULL)
    {
        printf("GetProcAddress failed for ntdll NtQuerySystemTime.\n\n");
    }
